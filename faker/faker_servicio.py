    ...
    for profile in profiles:
        ...
        for llave, valor in list(profile.items()):
            if llave == "current_location":
                ...
                profile.update(
                    current_location=json.dumps({"coordinates": coordinates})
                )
...
def execute_queries(list_queries_string=[], querie_type=""):
    ...
    db_host = "database-servicio"  # este es el servicio database declarado en el docker-compose
    db_port = "5432"
...
def create_profiles():
    print("CREATE RECORDS FAKER SERVICE")
    try:
...
def get_profiles():
    print("GET RECORDS FAKER SERVICE")
...
def delete_profiles():
    print("DELETE RECORDS FAKER SERVICE")
